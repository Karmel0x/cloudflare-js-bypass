...
inline std::string solveCFJSChallenge(std::string response, std::string url){//cloudflare
	...
	for (;;){
		...
		for (size_t i = 0; i < sa.size(); ++i)
			if (sb[i] == true) sprintf(bufferk, "%s\%d", bufferk, sa[i]);

		int sep_val;
		sscanf(bufferk, "%d", &sep_val);
